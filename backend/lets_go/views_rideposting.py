    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                driver = UsersData.objects.get(id=driver_id)
    ...
    if request.method == 'GET':
        ...
        for trip in trips:
            ...
            trip_list.append({
                ...
                'origin': route.first_stop.stop_name if route.first_stop else route.route_name,
                'destination': route.last_stop.stop_name if route.last_stop else route.route_name,
                ...
                'gender_preference': trip.gender_preference,  # Use the dedicated field
                'total_seats': trip.total_seats,
                'estimated_arrival_time': str(trip.estimated_arrival_time),
                'notes': trip.notes,
                'is_negotiable': trip.is_negotiable,  # Add negotiation flag
                # Enhanced data
    ...
    if request.method == 'GET':
        try:
            ...
            for trip in trips:
                ...
                route_names = [stop.stop_name for stop in route_stops]
                
                # Get booking count
                booking_count = trip.trip_bookings.filter(booking_status='CONFIRMED').count()
                ...
                # Get stop breakdown for detailed pricing
                stop_breakdown = []
                for breakdown in trip.stop_breakdowns.all().order_by('from_stop_order'):
                    stop_breakdown.append({
                        'from_stop_name': breakdown.from_stop_name,
                        'to_stop_name': breakdown.to_stop_name,
                        'distance': float(breakdown.distance_km),
                ...
                ride_data = {
                    ...
                    'distance': float(trip.total_distance_km) if trip.total_distance_km else None,
                    'duration': trip.total_duration_minutes,
                    'custom_price': float(trip.base_fare),
                    'total_seats': trip.total_seats,
                    'available_seats': trip.available_seats,
                    'booking_count': booking_count,
                    'gender_preference': trip.gender_preference,  # Use dedicated field
                    'description': trip.notes if trip.notes else '',  # Keep notes as description
