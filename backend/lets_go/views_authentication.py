...
def get_user_data_dict(request, user):
    ...
    for field in image_fields:
        if getattr(user, field):
            data[field] = f"http://192.168.100.10:8000/lets_go/user_image/{user.id}/{field}/"
        else:
            data[field] = None
    # Add vehicles if any
    vehicles = []
    if hasattr(user, 'vehicles'):
        for v in user.vehicles.all():
            vehicle_data = {
                ...
                'photo_front': f'http://192.168.100.10:8000/lets_go/vehicle_image/{v.id}/photo_front/' if v.photo_front else None,
                'photo_back': f'http://192.168.100.10:8000/lets_go/vehicle_image/{v.id}/photo_back/' if v.photo_back else None,
                'documents_image': f'http://192.168.100.10:8000/lets_go/vehicle_image/{v.id}/documents_image/' if v.documents_image else None,
            }
...
def login(request):
    if request.method == 'POST':
        ...
        try:
            user = UsersData.objects.get(email=email)
            if check_password(password, user.password):
                request.session['user_id'] = user.id
                print(f"user_id: {user.id}")
                user_data = get_user_data_dict(request, user)
                return JsonResponse({'success': True, 'message': 'Login successful', 'UsersData': [user_data]})
            else:
...
def user_image(request, user_id, image_field):
    ...
    try:
        ...
        user = UsersData.objects.get(id=user_id)
        image_data = getattr(user, image_field)
    ...
    try:
        ...
        for v in user.vehicles.all():
            vehicles.append({
                ...
                'insurance_expiry': str(v.insurance_expiry) if v.insurance_expiry else None,
                # Add photo URLs
                'photo_front': request.build_absolute_uri(f'/lets_go/vehicle_image/{v.id}/photo_front/') if v.photo_front else None,
                'photo_back': request.build_absolute_uri(f'/lets_go/vehicle_image/{v.id}/photo_back/') if v.photo_back else None,
            })
